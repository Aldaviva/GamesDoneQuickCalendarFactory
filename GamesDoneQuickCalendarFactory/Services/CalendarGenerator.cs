                ...
                if (runIndex == 0) {
                    calendarEvent.Alarms.AddAll(
                        new Alarm {
                            ...
                            Trigger     = new Trigger(((Duration) (Days) (-7)).ToIcalDuration()),
                            Description = $"{gdqEvent.longTitle} is coming up next week"
                        }, new Alarm {
                            Action      = AlarmAction.Display,
                            Trigger     = new Trigger(((Duration) (Days) (-1)).ToIcalDuration()),
                            Description = $"{gdqEvent.longTitle} is starting tomorrow"
                        }, new Alarm {
                            Action      = AlarmAction.Display,
                            Trigger     = new Trigger(((Duration) (Minutes) (-15)).ToIcalDuration()),
