...
/// <param name="tags"><para>Zero or more of <c>awful</c>, <c>bingo</c>, <c>bonus</c>, <c>btb_finale</c>, <c>btb_kickoff</c>, <c>btb_studio</c>, <c>checkpoint</c>, <c>checkpoint_run</c>, <c>chomp</c>, <c>coop</c>, <c>cw_lights</c>, <c>cw_motion</c>, <c>finale</c>, <c>flame_kickoff</c>, <c>frost_kickoff</c>, <c>gdqx</c>, <c>horror</c>, <c>kaizo</c>, <c>kickoff</c>, <c>mystery</c>, <c>new_addition</c>, <c>new_highlighted</c>, <c>online</c>, <c>opener</c>, <c>preshow</c>, <c>race</c>, <c>randomizer</c>, <c>recap</c>, <c>relay</c>, <c>rhythm</c>, <c>rundown</c>, <c>science</c>, <c>showcase</c>, <c>silly</c>, <c>sleep</c>, <c>speedrun</c>, <c>studio</c>, <c>tas</c>, and <c>tournament</c> (taken on 2026-03-14 from all runs in all events, but there can be more in the future). Can be empty, but never null.</para><para>JSON Query for an array of runs: <c>map(.tags) | flatten() | uniq() | sort()</c></para></param>
public sealed record GdqRun(
    int id,
